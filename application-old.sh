...
#启动的环境
ACTIVE_PROFILE=dev/uat/prd

#java虚拟机启动参数
JAVA_OPTS="-Xms512m -Xmx2048m -Xmn512m -Djava.awt.headless=true"
...
checkpid() {
   javaps=`$JAVA_HOME/bin/jps -l | grep $APP_HOME$APP_NAME`
   if [ -n "$javaps" ]; then
...
###################################
log() {
   tail -f /u01/logs/${APP_NAME%.jar}.log
}
